...
// Inisialisasi Firebase (Cukup 1 kali saja)
...
loginForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    console.error(error);
    loginMessage.textContent = getFirebaseError(error);
  }
});
...
dataForm.addEventListener("submit", async (event) => {
  ...
  if (!user) {
    saveMessage.textContent = "Silakan login terlebih dahulu.";
    return;
  }
  ...
  try {
    ...
    console.error(error);
    saveMessage.textContent = "Gagal menyimpan data.";
  }
});
...
function renderData(data) {
  ...
  activeData.textContent = data.filter(item => item.active === true).length;
...
window.deleteData = async function(id) {
  ...
  try {
    await deleteDoc(doc(db, "dashboardData", id));
  } catch (error) {
    console.error(error);
...
function setConnection(online) {
  if (online) {
    ...
    connectionText.textContent = "REALTIME ONLINE";
  } else {
